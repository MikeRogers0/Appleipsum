...
function select_randomly(hash, amount){
  tmp_array = [];
  ...
  for(var i=0; i < amount; i++){
    tmp_array.push(hash.pop())
  }

  return tmp_array;
}

function update_ipsum(element){
  hash = ipsumQuotes.slice(0);
  amount = 4;

  if(element.attr('id') == 'keynotes'){
    hash = ipsumKeynotes.slice(0);
    amount = 3;
  } 

  ipsum = select_randomly(hash, amount);

  element.html('');
  for(var i = 0; i < ipsum.length; i++){
    element.append('<p>' + ipsum[i] + '</p>')
  }
}
